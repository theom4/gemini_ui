            ...
            <div className="grid grid-cols-1 lg:grid-cols-4 gap-6 mb-8">
                ...
                <div className="lg:col-span-1 card-depth p-6 rounded-2xl flex flex-col justify-between">
                    ...
                    <div className="relative w-56 h-56 mx-auto flex items-center justify-center">
                        <ResponsiveContainer width="100%" height="100%">
                            <PieChart><Pie data={draftData} innerRadius={75} outerRadius={90} dataKey="value" stroke="none">{draftData.map((_, index) => <Cell key={`cell-${index}`} fill={draftColors[index]} />)}</Pie></PieChart>
                        </ResponsiveContainer>
                        <div className="absolute inset-0 flex items-center justify-center flex-col z-20">
                            <span className="text-5xl font-light dark:text-white font-num glow-text">{displayValue(draftConversionRate.toFixed(2))}%</span>
                        </div>
                    </div>
                </div>
                <div className="lg:col-span-1 card-depth p-6 rounded-2xl flex flex-col justify-between">
                    ...
                    <div className="relative w-56 h-56 mx-auto flex items-center justify-center">
                        <ResponsiveContainer width="100%" height="100%">
                            <PieChart><Pie data={conversionData} innerRadius={75} outerRadius={90} dataKey="value" stroke="none">{conversionData.map((_, index) => <Cell key={`cell-${index}`} fill={conversionColors[index]} />)}</Pie></PieChart>
                        </ResponsiveContainer>
                        <div className="absolute inset-0 flex items-center justify-center flex-col z-20">
                            <span className="text-5xl font-light dark:text-white font-num glow-text">{displayValue(conversionRate.toFixed(2))}%</span>
                        </div>
                    </div>
                </div>
            </div>
                            ...
                            {recordingsLoading ? <tr><td colSpan={6} className="py-8 text-center text-gray-500 italic">Se încarcă...</td></tr> :
                                ...
                                recentRecordings.map((rec) => (
                                    <tr key={rec.id} className="group hover:bg-white/5 transition-colors border-b border-gray-800/50 last:border-0">
                                        ...
                                        <td className="py-4 px-4 text-right"><audio controls className="h-6 w-32 inline-block opacity-40 hover:opacity-100 transition-opacity" src={rec.recording_url} /></td>
                                    </tr>
                                ))
                            }
                        </tbody>
